...
from lbg.utils.pose_util import (
...
# get_real_umi_obs_dict 函数的作用是：
# 该函数根据 shape_meta 的描述，将环境（env_obs）中的观测数据（包括多机器人ee（末端执行器）的位置和姿态、相机rgb图像、低维状态等）转换为统一的字典（obs_dict_np），
# 其中图像会按照shape_meta的分辨率resize整理（并归一化到[0,1]），低维数据会直接提取，同时充分支持多机器人场景。
# 此外，对于多机器人系统，如果指定，函数还可根据姿态表示模式(obs_pose_repr)和配对变换(tx_robot1_robot0、episode_start_pose)生成相对位姿，为后续强化学习/模仿学习兼容标准观测格式做准备。
...
def get_real_umi_obs_dict(
    ...
    for key, attr in obs_shape_meta.items():
        ...
        if type == 'rgb':
            ...
            #debug
            # print("Current env_obs keys:", list(env_obs.keys()))
            # print(env_obs["robot0_eef_pos"])
            # print(env_obs["robot0_eef_rot_axis_angle"])

            # for key, attr in obs_shape_meta.items():
            #     print(f"key: {key},  attr: {attr}")
            # #exit()

            # Skip keys that don't exist in env_obs (will be generated later)
